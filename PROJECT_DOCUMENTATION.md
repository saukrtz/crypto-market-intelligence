# Crypto Market Intelligence Pipeline — Project Documentation

> **Authors:** Ronit Kochar / Sigmoid Team
...
### 7.1 Add Unit and Integration Tests
The `requirements.txt` includes `pytest` and `responses` but no test files exist. Should add:
- Mocked API response tests for both ingestors.
- Spark DataFrame assertion tests for transformations.

### 7.2 Switch to Incremental Loads
Silver currently uses `mode("overwrite")`, reprocessing everything each run. Switching to Delta `merge` (upsert) with watermarking would be far more efficient at scale.

### 7.3 Add Monitoring and Alerting
Integrate Slack/email notifications via Airflow callbacks for failed tasks, empty payloads, or data quality issues.

### 7.4 Implement Data Validation Framework
Replace manual Bronze validation with Great Expectations — automated quality checks with formal expectations, reports, and alerting.

### 7.5 Build CI/CD Pipeline
GitHub Actions to automate Docker image builds, push to a container registry, run tests, and deploy K8s manifests on merge.

### 7.6 Upgrade Secret Management
Replace `.env` files and basic K8s Secrets with Azure Key Vault for production-grade credential management.

### 7.7 Add Streaming Ingestion
Replace the 15-minute batch with Kafka + Spark Structured Streaming for true sub-second real-time processing.

### 7.8 Implement ML Models
The `ml_models/` directory is currently empty. Future work: price prediction using Gold features (moving averages, volatility, volume spikes) as input features.

### 7.9 Add API Caching and Pagination
The FastAPI service reads the full Delta table per request. Redis caching and pagination would dramatically improve performance.

### 7.10 Implement Deduplication
The README mentions deduplication by `id` and `ingested_at`, but the Silver notebook doesn't call `dropDuplicates()`. This should be added.

### 7.11 Parameterize Hardcoded Values
Storage account name (`coinmarketcapapi`), container name, and Databricks job ID are hardcoded. These should be environment variables or Airflow Variables.

### 7.12 Complete the API Deployment Manifest
`infra/api-deployment.yaml` is empty. A K8s Deployment + Service manifest should be written to deploy the FastAPI service alongside Airflow on Minikube.
